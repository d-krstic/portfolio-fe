...
const globalSlice = createSlice({
  ...
  reducers: {
    ...
    removeSnackbarWithAction(state, action: PayloadAction<string>) {
      state.snackbars = state.snackbars.filter(
        (snackbar) => snackbar.actionId !== action.payload
      );
    },
    ...
    startLoading(state, action: PayloadAction<LoadingAction>) {
      ...
      state.errorActions = state.errorActions.filter((savedAction) =>
        filterAction(savedAction, action)
      );
    },
    stopLoading(state, action: PayloadAction<LoadingAction>) {
      state.loadingActions = state.loadingActions.filter((savedAction) =>
        filterAction(savedAction, action)
      );
    },
    addError(state, action: PayloadAction<ErrorAction>) {
      state.errorActions.push(action.payload);
    },
