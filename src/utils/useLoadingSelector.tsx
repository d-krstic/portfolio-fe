...
import { removeError } from 'src/store/features/globalSlice';
import { ApiError, LoadingAction } from 'src/store/models/Action';
...
export const useLoadingSelector = (action: LoadingAction): boolean => {
  ...
  const loading = useAppSelector((state) =>
    state.global.loadingActions.some(
      (ac) =>
        ac.actionType === action?.actionType &&
        (action?.id ? ac.id === action.id : true)
    )
  );
...
export const useErrorSelector = <T,>(action: LoadingAction): ApiError<T> => {
  ...
  return getErrors<T>(errorAction?.error);
};
